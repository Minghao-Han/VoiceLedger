...
这套还没经过大量实测对比。SenseVoice 官方数据是非自回归、比 Whisper 快 5 倍、234M 参数，中文场景理论上应该比同量级 Whisper 更准，但实际效果需要在真机上跟 whisper.rn 那边对比着测。如果测下来效果不如 whisper.rn，把 `src/app/index.tsx` 里的 `VOICE_ENGINE` 改回 `'whisper'` 就行，两套代码都还在，互不影响。
